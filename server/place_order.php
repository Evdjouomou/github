...
if(isset($_POST['place_order'])){
    ...
    $stmt= $conn->prepare("INSERT INTO orders(order_cost, order_statut, user_id, user_phone, user_city, user_adress, order_date) VALUES (?,?,?,?,?,?,?);");
    ...
    foreach($_SESSION['cart'] as $key =>$value){
        ...
        //4. store each single item in order_0items database
        $stmt1 = $conn->prepare("INSERT INTO order_items (order_id, product_id, product_name, product_image, user_id, order_date,product_price,product_quantity) VALUES(?,?,?,?,?,?,?,?)");
